//Version 1.5. Use g++ -odiffused lattice_hydro_fft_diffusedN16.cpp -lfftw3 -lm -lstdc++ -fopenmp
...
parse(int argc, char* argv[])
{
  try
  {
    ...
    .add_options()
    ("h, hsize", "h value (lattice spacing) to use for the fine grid.", cxxopts::value<double>(), "h")
...
//Leray projection using fft.
void proj(double deg1ch[], double out[], double pressure[], double **pressure8, fftw_plan &fft3d, fftw_plan &ifft3d){
  bd10(deg1ch,pressure);
  split8(pressure,pressure8);
  double s1,s2,s3,iev;


  fftw_complex **fftp= new fftw_complex*[8]; //temporary variables to store fft of the pressure

  for(int octal=0; octal<8; octal++) fftp[octal]= (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * OCTALSIZE);

  #pragma omp parallel for private(s1,s2,s3,iev)
  for(int octal=0; octal<8; octal++) {
    fftw_execute_dft_r2c(fft3d, pressure8[octal], fftp[octal]);


    for (int i = 0; i < LL; i++) { for (int j = 0; j < WW; j++) { for (int k = 0; k < (HH/2+1); k++) {
      if ((i==0)&&(j==0)&&(k==0)) {iev=0;}
      else { s1=sin(i*M_PI/LL);
        s2=sin(j*M_PI/WW);
        s3=sin(k*M_PI/HH);
        iev = 1/(factor*(s1*s1+s2*s2+s3*s3));}
        fftp[octal][i*WW*(HH/2+1) + j*(HH/2+1) + k][0] *= iev;
        fftp[octal][i*WW*(HH/2+1) + j*(HH/2+1) + k][1] *= iev;
      }}}

      fftw_execute_dft_c2r(ifft3d, fftp[octal], pressure8[octal]);
    }

    unsplit8(pressure8,pressure);
    d01(pressure,out);

    for(int i=0; i<ARR_SIZE; i++) out[i] = deg1ch[i] - out[i];

    for (int octal=0; octal<8; octal++) fftw_free(fftp[octal]);
    delete[] fftp;
  }
...
// Computes proj(Vf wedge vf) - nu*Laplacian(V), where proj is the Leray projection.
  inline void nav_stoke (double h,double nu,double V[], double Vfvf_res[], double out[], double pressure[], double **pressure8, fftw_plan &fft3d, fftw_plan &ifft3d) {
      ...
      proj(Vfvf_res, out, pressure, pressure8, fft3d, ifft3d);
    }
...
int main(int argc, char *argv[])
    {
      ...
      double **scal8 = new double*[8];
      for (int octal=0; octal<8; octal++) scal8[octal] = new double[OCTALSIZE];
      double id, jd, kd;
      ...
      if (result.count("h")) h = result["h"].as<double>();
      ...
      if (!silent) {
        std::cout<<"Using values : h "<<h<<", nu "<<nu<<", eta "<<eta<<std::endl;
      }
...
*/
      ...
      // fft_in and fft_out are temporary variables in order to construct the fftw plans.
      double *fft_in = new double[OCTALSIZE];
      fftw_complex *fft_out;
      fftw_plan fft3d, ifft3d;

      fft_out = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * OCTALSIZE);

      fft3d = fftw_plan_dft_r2c_3d(LL,WW,HH, fft_in, fft_out, FFTW_PATIENT);
      ifft3d = fftw_plan_dft_c2r_3d(LL,WW,HH, fft_out, fft_in, FFTW_PATIENT);

      // erase the variables as fftw plans are constructed.
      delete [] fft_in;
      fftw_free(fft_out);
      ...
      for(int i=0; i<LL; i++) {
        for(int j=0; j<WW; j++) {
          for(int k=0; k<HH; k++) {
            ...
            tempC[I(X,i,j,k,LL,WW,HH)] = MU*exp(-eta*((id*id)+(jd*jd)+(kd*kd)));
          }
        }
...
cout<<"L_inf norm of div V is: "<<maxnormscal(scal)<<endl;
      ...
      for (int rep=0; rep<10001; rep++) {
		...
		// Runge Kutta
        nav_stoke(h,nu,V, Vfvf_res, R, scal, scal8, fft3d, ifft3d); //scal is pressure.
        ...
        nav_stoke(h,nu,temp, Vfvf_res, R, scal, scal8, fft3d, ifft3d);
        for(int i = 0; i<ARR_SIZE; i++) temp[i] = V[i] + R[i]*dt*0.5;
   ...
   if (rep%50==0) {
        ...
        write2fileC(maximumVC, gradnormC);}
      }
